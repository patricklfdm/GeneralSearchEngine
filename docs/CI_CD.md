...
the eight full CI gates. Pull requests compare merge-base to PR head, so a final docs
...
the lightweight V5.0/V5.1 contract checks and `Required` run. Maven, Java setup and the eight
...
is always reported. It requires successful change detection and either all eight
full gates to succeed, or all eight to be intentionally skipped for a verified
...
For build inputs or manual dispatch, the workflow runs eight independent gates after
...
2. `v51-regression` (`V5.1 regression`) builds and tests its own reactor, then runs
   every V5.1 foundation, storage, protocol, runtime, rejoin, public qualification
   and bootstrap gate, retaining the corresponding evidence.
3. `v50-regression` (`V5.0 regression`) builds and tests its own reactor, then runs
   public admission, offline authority, public runtime and Phase 1–6 gates, including
   local cloud/remote workload qualification and all existing evidence uploads.
4. `v4-regression` (`V4 regression`) compiles its own core and test harnesses, then
   retains the V4.0–V4.4 chain, specialized JMH rebuilds and closed-line conditions.
5. `soak-examples` (`Soak and examples`) runs the JMH instrumentation contract tests,
   reduced stabilization/JFR end-to-end checks and travel example. These commands
   build their own prerequisites.
6. `Compatibility` runs the frozen source/reflection fixture, compares the public API
...
7. `Release artifacts` builds sources and strict Javadocs with GPG intentionally
...
8. `Cloud runner (no GCP)` validates shell syntax, the manual cloud-performance
