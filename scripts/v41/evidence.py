#!/usr/bin/env python3
"""Canonical checksummed V4.1 operational-safety evidence bundles."""
...
SCHEMA = "gse-v41-operational-evidence-v1"
CHECKSUMS = "artifact-checksums.sha256"
MAX_BYTES = 1024 * 1024
SOURCE = re.compile(r"[0-9a-f]{40}")
TOP_LEVEL = {
    "schemaVersion", "kind", "status", "sourceCommit", "sourceState",
    "suite", "preset", "profile", "case", "configuration", "backup",
    "verification", "restore", "process", "lifecycle", "cleanup", "logs",
    "result",
}


class EvidenceError(ValueError):
    """Evidence is missing, malformed, unbounded, or not internally consistent."""
...
def validate_bundle(directory: Path) -> dict[str, Any]:
    ...
    if document["suite"] != "v4.1-operational-safety-suite-v1" or \
            document["preset"] != "v4.1-operational-safety-v1":
    ...
    for field in ("case", "configuration", "backup", "verification", "restore",
                  "process", "cleanup", "logs", "result"):
    ...
    if not isinstance(limit, int) or not 0 < limit <= 65536:
    ...
    if document["status"] == "PASS" and (
            cleanup.get("status") != "PASS" or cleanup.get("leftovers") != []):
