...
invalid_image_error="$work_root/invalid-image.stderr"
if GSE_V44_GCP_PROJECT=gse-benchmark \
    ...
    GSE_V44_OUTPUT="$work_root/rejected" \
        scripts/v44/run_final_durable_cloud_member.sh --dry-run \
        2>"$invalid_image_error"; then
    echo "unfrozen cloud image was accepted" >&2
    exit 1
fi
grep -Fx \
    "ERROR: GSE_V44_CLOUD_IMAGE must be ubuntu-2404-noble-amd64-v20260906" \
    "$invalid_image_error" >/dev/null
