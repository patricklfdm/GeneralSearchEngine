...
cd "$root"
root_args=()
if [[ $# -eq 1 && "$1" == --allow-sudo-namespace ]]; then
  root_args+=(--allow-sudo-namespace)
elif [[ $# -ne 0 ]]; then
  echo "usage: $0 [--allow-sudo-namespace]" >&2
  exit 2
fi
python3 -m unittest scripts.v51.test_cloud_provider scripts.v51.test_cloud_package scripts.v51.test_guest_setup scripts.v51.test_guest_startup scripts.v51.test_guest_delivery scripts.v51.test_guest_deadline scripts.v51.test_guest_root_admission scripts.v51.test_guest_root
