...
"$python_command" -m unittest scripts.v50.test_performance_evidence
mkdir -p target/v50-performance
work_parent=$(mktemp -d "$root/target/v50-performance/run.XXXXXX")
echo "v50Phase6Evidence=$work_parent/evidence"
...
"$python_command" -m scripts.v50.test_performance_evidence --evidence "$work_parent/evidence" --output "$work_parent/semantic-negatives.json"
echo 'v50Phase6A=PASS execution=local-public-runtime-only'
