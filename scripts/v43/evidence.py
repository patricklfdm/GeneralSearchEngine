#!/usr/bin/env python3
"""Canonical checksummed V4.3 fast-reopen Phase 1 evidence bundles."""
...
SCHEMA = "gse-v43-fast-reopen-evidence-v1"
...
TOP_LEVEL = {
    ...
    "suite", "preset", "profile", "case", "configuration", "canonical",
    "derived", "process", "lifecycle", "cleanup", "logs", "result",
}
...
def validate_bundle(directory: Path) -> dict[str, Any]:
    ...
    for field in ("case", "configuration", "canonical", "derived", "process",
                  "cleanup", "logs", "result"):
    ...
    if document.get("status") == "PASS":
        if document["canonical"].get("authority") != "VALID":
            raise EvidenceError("passing evidence requires canonical authority")
...
def base_document(source: str, source_state: str, profile: str) -> dict[str, Any]:
    return {
        ...
        "configuration": {"productionFormat12": False,
                          "productionDerivedState": False, "paidExecution": False},
