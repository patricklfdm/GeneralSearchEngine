"""One evidence-bound foundation gate; no enabled automatic runtime or paid cloud."""
import argparse
import hashlib
import json
...
def run(output,control_directory):
    ...
    record=dict(schema='gse-v51-foundation-receipt-v1',source=source,diffSha256=hashlib.sha256(changed).hexdigest(),
                untrackedSha256=untracked,execution='independent-foundation-only',runtimeEnabled=False,paidCloud=False,status='FAIL')
    try:
...
if __name__=='__main__':
    ...
    r=run(a.output,a.controls);print(json.dumps({k:r[k] for k in ('status','execution','runtimeEnabled','paidCloud')}))
