...
class Group:
    def __init__(self, run, cp, java, plan):
    ...
    def offline(self, command, *extra):
        return self.run.process('offline-'+command+'-'+ '-'.join(map(str, extra)),
            [*self.java, '-cp', self.cp, PACKAGE+'admission.V50CloudWorkloadConsumer', *self.args(0),command,*map(str,extra)],True)
    def start(self, ordinal):
        ...
        w=Worker(self.run,label,[*self.java,'-cp',self.cp,PACKAGE+'replication.V50CloudWorkloadWorker',*self.args(ordinal),str(output)])
    ...
    def capture(self,label):
        ...
        reports=[]
        for i in (1,2,3):
            shutil.copytree(self.root/f'node-{i}',path/f'node-{i}')
...
def run(root,control):
    ...
    try:
        ...
        metadata=dict(head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
            ...
            arithmetic=arithmetic(plan),java=runner.process('java-version',['java','--version']),
        ...
        classes=raw/'classes-candidate';classes.mkdir();oracle=raw/'classes-control';oracle.mkdir()
        ...
        group=Group(runner,cp,java,plan);group.offline('bootstrap');group.start_all();group.command('activate')
        cycle=0
        def healthy():
            nonlocal cycle
            windows=[]
        ...
        def restart():
            ...
            genesis=f.genesis((raw/'node-1/genesis.gsr').read_bytes());manifest=f.manifest((raw/'node-1/manifest.gsr').read_bytes(),genesis)
        ...
        def replace(target,source):
            ...
            group.close();(raw/f'node-{target}').rename(raw/f'lost-node-{target}');group.offline('replace',target,source)
            if target==3:
        ...
        def capacity():
            ...
            for attempt in range(3):
                ...
                shutil.copytree(raw/'node-1',before_path)
                ...
                if not r['accepted']:
                    ...
                    after_path=raw/'capacity-sources'/'after-rejection';shutil.copytree(raw/'node-1',after_path)
        ...
        actual=source_inputs();actual[str(PLAN.relative_to(ROOT))]=sha_file(PLAN);check(actual==inputs,'inputs changed during qualification')
...
if __name__=='__main__':
    ...
    a=p.parse_args();run(a.workspace,a.control_jar)
